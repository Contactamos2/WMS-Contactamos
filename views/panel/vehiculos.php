		...
		<div class="caja-min caja-form animatetop">
			<form id="form" method="post" enctype="multipart/form-data">
				...
				<div class="caja-form-header">
					...
					<h2><i class="fas fa-truck-moving"></i>Gestión de Vehículo</h2>
				</div>
				<div class="caja-form-section">
					<div class="caja-fila-inputs">
						<div class="caja-inputs">
							<label>Vehículo</label>
							<input type="text" id="vehiculo" name="vehiculo" onChange="this.value=(this.value).toUpperCase()" style="text-transform:uppercase" required>
						</div>
						<div class="caja-inputs">
							<label>Tiempo máximo E1</label>
							<input type="number" id="e1" name="e1" min="0" required>
						</div>
						<div class="caja-inputs">
							<label>Tiempo máximo E2</label>
							<input type="number" id="e2" name="e2" min="0" required>
						</div>
						<div class="caja-inputs">
							<label>Tiempo máximo E3</label>
							<input type="number" id="e3" name="e3" min="0" required>
						</div>
					</div>		
				</div>
			...
			<div class="caja-scroll">
				<table id="tabla">
					<thead>
						<tr>
							<th><a>Opciones</a></th>
							<th><a href="Javascript:void()" onClick="w3.sortHTML('#tabla', '.fila', 'td:nth-child(2)')">Vehículo</a></th>
							<th><a href="Javascript:void()" onClick="w3.sortHTML('#tabla', '.fila', 'td:nth-child(5)')" title="Tiempo máximo">TM E1</a></th>
							<th><a href="Javascript:void()" onClick="w3.sortHTML('#tabla', '.fila', 'td:nth-child(6)')" title="Tiempo máximo">TM E2</a></th>
							<th><a href="Javascript:void()" onClick="w3.sortHTML('#tabla', '.fila', 'td:nth-child(7)')" title="Tiempo máximo">TM E3</a></th>
						</tr>
					</thead>
					<tbody></tbody>
				</table>
...
function tablear(){
	...
	$.ajax({
		url: "tables/vehiculos_filas.php",
		type: "POST",
		dataType: "HTML",
	...
	$.ajax({
		url: "tables/vehiculos_tabla.php",
		type: "POST",
		dataType: "HTML",
...
function editar(b){
	...
	.done(function(info){
		...
		var carpado = info[0].carpado;
		var precarga = info[0].precarga;
		var e1 = info[0].e1;
		var e2 = info[0].e2;
		var e3 = info[0].e3;
...
$("#form").on("submit", function(e){
	...
	$.ajax({
		url: "models/guardar_vehiculo.php",
		type: "POST",
		dataType: "html",
		data: formData,
		cache: false,
