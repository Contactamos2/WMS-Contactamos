...
if($resultado1 == true){
	...
	while($fila1 = $resultado1->fetch_array()){
		...
		$sql2 = "SELECT f.atencion, f.toneladas
				 FROM procesos AS f
		...
		if($resultado2 == true){
			...
			while($fila2 = $resultado2->fetch_array()){
				...
				if($atencion == "DESCARGUE"){ $descargues += $toneladas/1000; }
				if($atencion == "CARGUE"){ $cargues += $toneladas/1000; }
		...
		$arreglo[] = array("registro"=>$registro,
				   		   "cargue"=>round($cargues,2),
				   		   "descargue"=>round($descargues,2));
